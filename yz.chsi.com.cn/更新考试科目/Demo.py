...
csv_data=pd.read_csv('school_info_li2.csv')

values=csv_data.values
i=0
dfdata = pd.DataFrame(
            {"学校":['学校'],"政治": ['政治'], "英语": ['英语'], "数学": ['数学'], "专业": ['专业']})
dfdata.to_csv('school_info_zy2_major.csv', index=False, encoding='utf_8_sig', mode='a', header=False)
for value in values:
    try:
        if i%100==1:
            time.sleep(2)
        ...
        dfdata.to_csv('school_info_li2_major.csv', index=False, encoding='utf_8_sig', mode='a', header=False)
    except:
        print("程序崩了")
        print(value)
        print("-------")
